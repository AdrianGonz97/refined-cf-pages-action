...
// node_modules/.pnpm/reghex@1.0.2/node_modules/reghex/dist/reghex-core.mjs
...
async function IfStatement(chunk, context4) {
  const { interps, last_cmd } = context4;
  ...
  if (interps[val_id]) {
    return execute(if_clause, context4);
  } else if (else_clause) {
    return execute(else_clause, context4);
  } else {
    return last_cmd;
  }
}
async function Command2(chunk, context4) {
  const { interps, cwd, shell, exit_expected } = context4;
  const [str] = chunk;
...
async function InStatement(chunk, context4) {
  const { interps } = context4;
  ...
  return execute(in_clause, {
    ...context4,
    cwd: import_path.default.resolve(context4.cwd, new_cwd)
  });
}
async function Grammar(chunk, context4) {
  const { last_cmd } = context4;
...
async function Await(chunk, context4) {
  const { interps, last_cmd } = context4;
...
async function Stdout(chunk, context4) {
  const { interps, last_cmd, captures } = context4;
...
async function Exitcode(chunk, context4) {
  const { interps, last_cmd } = context4;
...
async function ExitsStatement(chunk, context4) {
...
var execute = async (chunk, context4) => {
  if (Array.isArray(chunk)) {
    if (chunk.tag === "command_line" || chunk.tag === "logged_command") {
      return Command2(chunk, context4);
    } else if (chunk.tag === "if_statement") {
      return IfStatement(chunk, context4);
    } else if (chunk.tag === "in_statement") {
      return InStatement(chunk, context4);
    } else if (chunk.tag === "grammar") {
      return await Grammar(chunk, context4);
    } else if (chunk.tag === "await_statement") {
      return await Await(chunk, context4);
    } else if (chunk.tag === "stdout_statement") {
      return await Stdout(chunk, context4);
    } else if (chunk.tag === "exitcode_statement") {
      return await Exitcode(chunk, context4);
    } else if (chunk.tag === "exits_statement") {
      return await ExitsStatement(chunk, context4);
    } else {
      return context4.last_cmd;
    }
  }
  return null;
};
